...
class RRTMotionPlanner(object):
    ...
    def extend_1(self, near_config, rand_config):
        if self.bb.config_validity_checker(np.array(rand_config)) and self.bb.edge_validity_checker(
                np.array(near_config[1]), np.array(rand_config)):
    ...
    def extend_2(self, near_config, rand_config):
        ...
        if length == 0:
            return
        extend_config = [(near_config[1][i]) + min(self.eta, length) * ((rand_config[i] - near_config[1][i]) / length)
                         for i in range(len(near_config[1]))]
        if self.bb.config_validity_checker(np.array(extend_config)) and self.bb.edge_validity_checker(
                np.array(near_config[1]), np.array(extend_config)):
